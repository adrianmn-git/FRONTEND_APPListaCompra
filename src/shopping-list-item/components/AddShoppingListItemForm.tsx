...
import { faPlus, faBasketShopping } from "@fortawesome/free-solid-svg-icons"
import CustomSelect from "@/components/ui/CustomSelect"
import { useI18n } from "@/i18n/hooks/useI18n"
import { addShoppingListItemSchema } from "../entity/schemas"
import { ZodError } from "zod"
import { UNIT_OPTIONS } from "../utils/unitConfig"
...
export default function AddShoppingListItemForm({ listId }: Props) {
  ...
  const handleOpen = async (open: boolean) => {
    setIsOpen(open)
    if (open && products.length === 0) await loadProducts()
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpen}>
      ...
      <DialogContent className="bg-white/95 backdrop-blur-xl rounded-[2.5rem] p-8 md:p-10 w-full max-w-md shadow-2xl border border-white/50 animate-in zoom-in-95 slide-in-from-bottom-8 duration-500 sm:rounded-[2.5rem]">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <Input
                ...
                className={cn(
                  "h-11 bg-slate-50 border-2 text-slate-800 font-bold px-5 transition-all rounded-xl shadow-sm",
